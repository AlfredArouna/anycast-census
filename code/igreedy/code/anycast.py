...
#-----------Graphs variable-----------
        ...
        for line in data.readlines():
            try:
                if line.startswith("#"):
                    continue
                #igreedy 1.3: save RTT as information
                ...
                minRTT=columns[3]
                TTL=-1
                if len(columns)>4: #now Ripe and Planetlab should provide the ttl
                    TTL=columns[4]
                    if len(columns)>5:
                       self._countryStatistic.append(columns[5].title())
                """ 
                #igreedy 1.2: understand how to improve
                columns=line.strip().split("\t")
                hostname=columns[0]
                latitude=columns[1]
                longitude=columns[2]
            ...
            if not ( float(minRTT) > threshold and threshold > 0):
                ...
                #computation data for the graphs
                self._allRadius.append((float(minRTT)*98.615940132)) #radius
                self._allTtl.append(int(TTL)) #TTL
                if(hostname.isdigit()):
                    self._ripeVp+=1
                else:
                    self._planetlabVp+=1
                ...
                if(self._setDisc.get(float(minRTT)) is None):
                    self._setDisc[float(minRTT)]=[Disc(hostname,float(latitude),float(longitude),float(minRTT),int(TTL))]
                else:                
                    self._setDisc[float(minRTT)].append(Disc(hostname,float(latitude),float(longitude),float(minRTT),int(TTL)))
        data.close()
...
#create a subset of airport inside the disk and after decide witch one is the one we guess
        ...
        return [self.geolocateCircle(disc,airportsInsideDisk),len(airportsInsideDisk)]
        """
...
#-----geolocation fig jsac
    ...
    def getAllRtt(self):
        return self._allRadius

    def getNumberPlanetLabVp(self):
        return self._planetlabVp

    def getNumberRipeVp(self):
        return  self._ripeVp

    def getCountryStatistic(self):
        return self._countryStatistic
