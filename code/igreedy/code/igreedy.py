        ...
        for radius, discList in resultEnumeration[1].getOrderedDisc().iteritems(): 
            for disc in discList:
                if(not disc[1]): #if the disc was not geolocated before, geolocate it!
                    ...
                    city,numberOfCityInsideTheCircle=anycast.geolocation(disc[0],treshold) #result geolocation
                    if(city is not False): #if there is a city inside the disc
                        ...
                        discsSolution.append((disc[0],city,numberOfCityInsideTheCircle)) #disc, marker
                        ...
                        discsSolution.append((disc[0],["NoCity",disc[0].getLatitude(),disc[0].getLongitude(),"N/A","N/A"],0)) #disc, marker                 
            if(iteration):
                break
...
def output():
    ...
    csv.write("#hostname\tcircleLatitude\tcircleLongitude\t" +\
              ...
              "city\tcode_country\tCityPopulation\tnumberOfCityInsideTheCircle\n")
    listSolutionTtl=[]
    listSolutionRtt=[]
    ...
    data.countGT=GTnum
    data.markerGT=[]
    data.countTotalNumberOfCircles=len(anycast.getDisc()) #all circles, now we are filtering the disks bigger than 6400Km
    data.allCircles=[]
    ...
    for instance in discsSolution:
            ...
            circle.ttl=tempCircle.getTtl()
            circle.numberOfCityInsideTheCircle=instance[2]
            #marker to Json
            ...
            marker.city_population=IATApop[tempMarker[0]]
            markCircle=Object()
            markCircle.marker=marker
            markCircle.circle=circle
            data.instances.append(markCircle)
            
    #empty circle: it filters the disc with a radius bigger than 6400 km(radius of the earth)
    count=+1
    for key,instance in anycast.getOrderedDisc().iteritems():
            #circle to Json
            tempCircle=instance[0]
            if(float(tempCircle.getRadius())<6400):
                circle=Object()
                circle.id= tempCircle.getHostname()
                circle.latitude= tempCircle.getLatitude()
                circle.longitude= tempCircle.getLongitude()
    ...
    data.countVisualisedCircles=len(data.allCircles)        
    if GTnum>0:
        for gt in GTtotal:
            markerGT=Object()
            markerGT.id= gt
...
#m should be parents or they have to share the same parameter
   ...
   outfile="-".join((values.output).split("-")[:-1])
